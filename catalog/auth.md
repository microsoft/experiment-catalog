...
The scheme resolution order is: `EXTERNAL_SCHEME` → `X-Forwarded-Proto` header → `Request.Scheme`. Setting `EXTERNAL_SCHEME` explicitly is the most reliable approach — it avoids dependence on forwarded headers that proxies may strip, overwrite, or that malicious clients could spoof.
